...
import static com.ringcentral.health.HealthCheckSignal.Type.*;
...
public class ReactiveHealthEngine implements HealthEngine {
    ...
    private void scheduleFirstChecks(HealthCheckConfig healthCheckConfig, HealthEngineConfig healthEngineConfig) {
        ...
        Observable<HealthCheckResultWrapper> filteredCheckResultObservable = Observable.merge(inputObservable, forceSubject, passiveObservable)
                ...
                .flatMap(go -> go.lift(new DistinctPassiveChecksOperator(log))); // this filters non-changing signals
    ...
    private HealthState fillInitialHealthState(HealthCheckFunction[] checkers, HealthCheckConfig healthCheckConfig) {
        if (checkers != null) {
            Instant instant = clock.instant();
            return new HealthState(Stream.of(checkers).collect(
                    Collectors.toMap(HealthCheckFunction::getId,
                            f -> new LatestHealthCheckState(
                                    HealthCheckResultWrapper.initial(f),
                                    instant,
                                    instant.plus(getExpirationPeriod(f.getId(), healthCheckConfig))
                            )
                    )
            ));
        } else {
            return new HealthState(Collections.emptyMap());
        }
    }
    ...
    private void processResult(HealthCheckResultWrapper result, HealthCheckConfig healthCheckConfig) {
        ...
        state.updateState(result, getExpirationPeriod(id, healthCheckConfig));
    }
